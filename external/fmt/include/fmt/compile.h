...
FMT_CONSTEXPR void compile_format_string(basic_string_view<Char> format_str, PartHandler handler) {
  parse_format_string<IS_CONSTEXPR>(format_str,
                                    format_string_compiler<Char, PartHandler>(format_str, handler));
}

template <typename Range, typename Context, typename Id>
void format_arg(basic_format_parse_context<typename Range::value_type> &parse_ctx,
                Context &ctx,
                Id arg_id) {
...
auto vformat_to(Range out, CompiledFormat &cf, basic_format_args<Context> args) ->
  ...
  for (auto part_it = std::begin(parts); part_it != std::end(parts); ++part_it) {
    ...
    case format_part_t::kind::replacement: {
      ...
      const auto arg = value.repl.arg_id.kind == arg_id_kind::index ? ctx.arg(arg_id_value.index)
...
struct compiled_format_base : basic_compiled_format {
  ...
  explicit compiled_format_base(basic_string_view<char_type> format_str) {
    compile_format_string<false>(
        format_str, [this](const format_part<char_type> &part) { compiled_parts.push_back(part); });
  }
...
constexpr auto compile(S format_str) {
  ...
  if constexpr (str.size() == 0) {
    ...
    constexpr auto result =
        internal::compile_format_string<internal::type_list<Args...>, 0, 0>(format_str);
