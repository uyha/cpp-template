...
inline std::size_t
strftime(wchar_t *str, std::size_t count, const wchar_t *format, const std::tm *time) {
...
public:
  ...
  FMT_CONSTEXPR auto parse(basic_format_parse_context<Char> &ctx) -> decltype(ctx.begin()) {
    ...
    format_str =
        basic_string_view<Char>(&*range.begin, internal::to_unsigned(range.end - range.begin));
    return range.end;
  }
